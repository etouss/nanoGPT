...
class HemisphereQwen2RotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None, config=None):
        ...
        bases_kv = torch.tensor([min_base, max_base], device=device).view(2, 1)
        self.bases_q = bases_kv.repeat_interleave(self.group_size, dim=0)
        self.bases_k = bases_kv

        # Compute Frequencies
        dim_indices = torch.arange(0, dim, 2, dtype=torch.float32, device=device)
        term = dim_indices / dim
        
        _inv_freq_q = 1.0 / (self.bases_q ** term)
        _inv_freq_k = 1.0 / (self.bases_k ** term)
...
# ==============================================================================
# 2. MONKEY PATCH FORWARD (Fixed Return Signature)
# ==============================================================================
def psystem_forward(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, output_attentions=False, use_cache=False, **kwargs):
    ...
    if hasattr(self, 'head_dim'): head_dim = self.head_dim
    else: head_dim = self.config.hidden_size // num_heads

    if hasattr(self, 'hidden_size'): hidden_size = self.hidden_size
    else: hidden_size = self.config.hidden_size
    ...
    # FIX: Correct Return Values for standard Qwen implementation
    # It expects 2 values if output_attentions is False
